...
// ── Bracket slot definitions (unchanged) ──────────────────────────────────────
const R32_SLOTS = {
...
const COL = CW + GAP; // 104
const SH  = 68;   // slot height for one R32 row
const H   = 8 * SH; // 544 — total bracket height

// Column x positions (left edge of each card)
const L_R32 = 0;
const L_R16 = L_R32 + COL;       // 104
const L_QF  = L_R16 + COL;       // 208
const L_SF  = L_QF  + COL;       // 312
const MG    = 36;                 // gap between SF and Final
const FINAL = L_SF  + CW + MG;   // 426
const R_SF  = FINAL + CW + MG;   // 540
const R_QF  = R_SF  + COL;       // 644
const R_R16 = R_QF  + COL;       // 748
const R_R32 = R_R16 + COL;       // 852
const TW    = R_R32 + CW;        // 930 — total SVG width

// ── Annexe C: official best-3rd-place assignment table ────────────────────────
// Key = sorted 8-group letters; Value = [M79, M85, M81, M74, M82, M77, M87, M80] (away slot groups)
// Columns: 1A(M79) 1B(M85) 1D(M81) 1E(M74) 1G(M82) 1I(M77) 1K(M87) 1L(M80)
const BEST3RD_TABLE = {
...
// Table columns → (matchNum, side) mapping
// Columns: [1A→M79, 1B→M85, 1D→M81, 1E→M74, 1G→M82, 1I→M77, 1K→M87, 1L→M80]
const BEST3RD_SLOTS = [
...
// Y-center of a match by round index
const yc = {
  r32: i => (i + 0.5) * SH,
  r16: i => (2 * i + 1) * SH,
  qf:  i => (4 * i + 2) * SH,
...
// ── Bracket tree order ────────────────────────────────────────────────────────
// Left half (top→bottom feeds SF M101 via QF M97/M98)
const LEFT = {
...
// Right half (top→bottom feeds SF M102 via QF M99/M100)
const RIGHT = {
...
// ── Official best-3rd-place assignment (Annexe C lookup) ─────────────────────
// Returns { matchNum: { side: team } } for all 8 best3rd slots, or null if
// fewer than 8 groups have played (can't determine the correct combination yet).
...
function BracketLines() {
  ...
  // Left half: connectors exit from right edge of card, go to next round's left edge
  function connectL(xOut, xIn, ysOut, ysIn) {
    const sx = xOut + CW + GAP / 2;
    for (let i = 0; i < ysIn.length; i++) {
      const yT = ysOut[2 * i];
      const yB = ysOut[2 * i + 1];
      const yM = ysIn[i];
      L(xOut + CW, yT, sx, yT); // right arm top
      L(xOut + CW, yB, sx, yB); // right arm bottom
      L(sx, yT, sx, yB);         // vertical spine
      L(sx, yM, xIn, yM);        // arm to next round
    }
  }

  // Right half: connectors exit from left edge of card, go to next round's right edge
...
export default function Bracket() {
  ...
  // Best-3rd assignments from Annexe C (official table, no duplicates)
  const best3rdMap = resolveBest3rdSlots(byGroup) || {};

  // Pre-compute projections for all unplayed matches
  const projMap = {};
  ...
  for (const num of Object.keys(R32_SLOTS).map(Number).sort()) {
    ...
    for (const side of ['home', 'away']) {
      ...
      if (slot.type === 'group') {
        projMap[num][side] = resolveTeam(slot, byGroup, dbByNum);
      } else {
        // best3rd: use official table assignment (null = TBD until 8 groups have played)
        projMap[num][side] = best3rdMap[num]?.[side] ?? null;
      }
    }
  }

  // Late-round projections (winner/loser from prior match results)
      ...
      {!groupStarted ? (
        ...
        <div className="overflow-x-auto pb-4">
          <div style={{ width: TW, minWidth: TW }}>
            ...
            {/* 3rd place (centered below bracket) */}
